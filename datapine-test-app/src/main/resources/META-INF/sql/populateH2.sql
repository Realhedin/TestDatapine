insert into users  values (1,'user@user.com','user');
insert into users  values (2,'admin@admin.com','admin');
insert into users  values (3,'acl@acl.com','acl');

insert into items values (1,'laptop',1);
insert into items values (2, 'mouse',1);
insert into items values (3, 'monitor',2);
insert into items values (4, 'cord',3);

-- insert into users  values (1,'user','user','ROLE_USER');
-- insert into users  values (2,'admin','admin','ROLE_ADMIN');